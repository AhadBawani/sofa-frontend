...
const CartCard = ({ data, disabled }) => {
    ...
    return (
        <div>
            {data?.map((item, index) => {
                return (
                    <div key={item?.productName + index} className="flex items-center m-1 p-2">
                        <div className="justify-start">
                            <img src={Request.PRODUCT_IMAGE + item?.productImage}
                                className="rounded-xl h-[70px] w-[80px]" alt={item?.productName} />
                        </div>
                        ...
                        <div className="flex space-x-2 ml-4">
                            {
                                disabled
                                    ...
                                    <IconButton size="small" disabled={item?.quantity === 1}>
                                        <AiOutlineMinus style={item?.quantity === 1 ?
